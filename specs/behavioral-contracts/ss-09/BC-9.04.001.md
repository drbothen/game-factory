...
The artifact must be structurally valid and complete before the distribution-readiness
convergence dimension can advance past AMBER.
...
4. The `convergence-report` for the game exists with a writable
   `dimensions.distribution_readiness` field.
...
4. Update `convergence-report.dimensions.distribution_readiness`:
   - `COMPLETE` → `GREEN`
   - `PARTIAL` → `AMBER`
   - `FAILED` → `BLOCKED`
   - `PENDING` → `AMBER`
...
- Every outstanding human-gated task is listed in `human_gated_tasks[]` — none are silently
  omitted (DI-006).
- `convergence-report.dimensions.distribution_readiness` reflects the computed status.
...
|-------|----------|----------------|
| EC-001 | No adapters invoked yet for this build version | `overall_distribution_status: PENDING`; `adapters_invoked: []`; `distribution_readiness: AMBER` |
