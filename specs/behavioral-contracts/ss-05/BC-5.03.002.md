...
3. Coverage check: for every AI-generated audio asset in `audio-build-manifest`, exactly
   one ledger entry exists. Missing entry → E-AUD-004.
...
5. If any AI-generated audio asset is missing from the ledger: E-AUD-004 raised for
   each missing asset.
...
2. (DI-009) Blocked-tool detection: any audio asset with `generated_by_tool` in the
   ...
   policy update) raises E-AUD-004 and is flagged as a compliance defect.
...
| EC-003 | Sidecar `generated_by_tool` is "Udio" (blocked tool) | E-AUD-004: blocked tool detected; asset flagged; ledger `blocked_tool_detected: true`; `ai-disclosure-manifest` generation blocked until resolved |
...
| 1 AI music asset with no provenance sidecar | E-AUD-004: asset '<id>' not covered by ai-audio-provenance-ledger | error |
...
|--------|----------|-------------|
| VP-5.03.004 | For all AI-generated audio assets in manifest, exactly one ledger entry exists | proptest: manifest with N AI assets → assert ledger.total_assets_covered = N |
| VP-5.03.005 | Blocked tool in any sidecar always triggers E-AUD-004 | proptest: inject Suno/Udio in tool field; assert error raised |
...
- VP-5.03.004 — ledger coverage completeness
- VP-5.03.005 — blocked-tool detection
