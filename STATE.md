...
**Current clean-pass counter: 0/3** (restart #3 still seeking clean #1). Next action: Pass 61 (candidate clean #1). Pass 60 FINDINGS (F60-01: VP-007 dual-invariant scope not propagated to verification-architecture.md:72 + BC-13.02.001:190 — fixed; F60-02: VP-004 over-strong win-only reachability corrected to terminal-set — fixed across 4 files; F60-03→FU-023 obs deferred).
...
2. After each fix burst: run `bash scripts/check-spec-counts.sh` (must exit 0 — gate v1.42, checks a–z + aa, bb, cc, dd, ee, ff, gg, hh, ii, jj, kk + o.ii, ~40 sub-assertions; check (gg) is D-SEC evaluator-completeness guard; check (hh) is economy-conservation BC-routing guard; check (ii) is visionOS/OpenXR dedicated-code routing guard; check (jj) is D-SEC no-DEGRADED-path consistency guard; check (kk) is W-code identifier-resolution guard). Machine-enforced spec-integrity gate. NEVER credit a pass without a green gate run.
...
**CI-gate contract:** `scripts/check-spec-counts.sh` v1.42 on `main` — checks a–z + aa, bb, cc, dd, ee, ff, gg, hh, ii, jj, kk + o.ii (~40 sub-assertions). Key guards: (ee) corpus-wide genre-profile schema; (ff) L2-INDEX 8 registry counts; (w) GENERAL DI-007 context (playtest keyword required); (gg) D-SEC evaluator-completeness (4 sub-predicates); (hh) economy-conservation BC routing; (ii) visionOS/OpenXR dedicated-code routing; (jj) D-SEC no-DEGRADED-path; (kk) W-code identifier-resolution guard. MUST exit 0 before crediting any pass.
...
| phase-1d-adversarial | Adversarial spec convergence (>=3 clean passes); Passes 1–60 resolved (see phase-1-log.md + convergence table below); 3 streak breaks at P46/49/51; security-burst tails at P43/44/53; latest: Pass 60 FINDINGS (F60-01: VP-007 dual-invariant scope — verification-architecture v1.2, BC-13.02.001 v1.2; F60-02: VP-004 terminal-set — VP-004 v1.2, VP-INDEX v1.6, coverage-matrix v1.3, BC-6.02.004 v1.1; F60-03→FU-023); **COUNTER: 0/3**; Pass 61 = restart #3 clean #1 candidate | IN PROGRESS |
...
| T9 | Phase-1d adversarial spec convergence — Passes 1–60 resolved; Pass 60 FINDINGS (F60-01: VP-007 dual-invariant scope — verification-architecture v1.2, BC-13.02.001 v1.2; F60-02: VP-004 terminal-set correction — VP-004 v1.2, VP-INDEX v1.6, coverage-matrix v1.3, BC-6.02.004 v1.1; F60-03→FU-023); counter 0/3; Pass 61 = restart #3 clean #1 candidate | **IN PROGRESS** |
...
| LESSONS-CODIFIED | LESSON-F43/F46/F49a/F49b/F52/F53/F56/F58 — all codified in CI checks dd/ee/ff/w/gg/jj/kk. LESSON-F60 [pending-codification] — VP re-scope must sweep all downstream surfaces. Full text: `.factory/cycles/v0.1.0-greenfield/lessons.md` | — | CODIFIED (LESSON-F60 pending gate) |
...
**Date:** 2026-06-13
**Phase:** 1 — Spec Crystallization IN PROGRESS. Steps 1–8 DONE + Phase-1d Passes 1–60 DONE.
**Phase-1d Pass 60:** FINDINGS (0C/2I/1obs). F60-01(I,process-gap): VP-007 F40-04 dual-invariant scope (INV-TS-02+INV-TS-03) not propagated to verification-architecture.md:72 (still showed stale single-invariant "μ−3σ monotonicity") + BC-13.02.001:190 VP Anchors back-ref — fixed: verification-architecture v1.1→v1.2, BC-13.02.001 v1.1→v1.2. F60-02(I): VP-004 property statement was over-strong ("goal state reachable from any valid state") contradicting BC-6.02.004 (softlock = cannot reach win OR game-over; EC-001 makes game-over-only states PASS) and BC-6.02.004/VP-TBD-015 — corrected to terminal-set reachability (win OR game-over); propagated to VP-004-no-softlock-reachability.md v1.1→v1.2, VP-INDEX v1.5→v1.6, verification-architecture v1.2 (covered), verification-coverage-matrix v1.2→v1.3, BC-6.02.004 v1.0→v1.1 (back-ref annotation only; softlock definition untouched). F60-03(obs→FU-023): verification-architecture.md:71 loose VP-006 paraphrase — deferred non-blocking.
**Clean-pass counter: 0/3 (counter stays — findings pass; restart #3 still seeking clean #1).** Pass 61 = restart #3 candidate clean #1.
**Next action:** `phase-1d-adversarial` — **Pass 61** (candidate clean #1, restart #3 continues). Spec FROZEN after F60 fix burst. FU-007/FU-008/FU-010/FU-011/FU-012/FU-013/FU-014/FU-016/FU-017/FU-018/FU-019/FU-020/FU-021/FU-022/FU-023/FU-024 open (non-blocking). FU-015 CLOSED (F51-02).
...
**Spec totals:** 193 BCs / 267 error codes (258 active) / 41 NFRs / 15 caps / 13 subsystems / 13 DI invariants / P0=127/P1=42/P2=24 / CI gate v1.42 (~40 sub-assertions). Error codes UNCHANGED (267); Warning Codes registry carries 4 W-codes (NOT counted in 267). §3.1 D-SEC = 2 allowed values (GREEN, BLOCKED); check (s) verifies 33 dim-value pairs.
**Version bumps Pass 60:** verification-architecture.md v1.2 (F60-01: VP-007 P1 table dual-invariant form; F60-02: VP-004 P0 table terminal-set); BC-13.02.001 v1.2 (F60-01: VP-007 VP Anchors back-ref dual-invariant); VP-004-no-softlock-reachability.md v1.2 (F60-02: property statement corrected to terminal-set); VP-INDEX.md v1.6 (F60-02: VP-004 Short Description + Summary Table + I2 table back-ref); verification-coverage-matrix.md v1.3 (F60-02: VP-004 row + BC-6.02.004 notes); BC-6.02.004.md v1.1 (F60-02: VP-004 back-ref annotation only). BC count UNCHANGED (193). Spec totals UNCHANGED. Gate v1.42 UNCHANGED (no main-branch edit). Gate codification candidate FU-024 deferred per LESSON-F52 (avoid over-firing).
...
**main branch:** UNCHANGED this pass (no gate-script edit). CI gate v1.42 on main. Step history: see phase-1-log.md.
