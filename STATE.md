...
**Current clean-pass counter: 0/3** (restart #3 still seeking clean #1). Next action: Pass 62 (candidate clean #1). Pass 61 FINDINGS (F61-01: F40-01 T2=snapshot-structured-diff not propagated to adapter-protocols.md §2.3 tier table + §2.5 ReplayResult enum — fixed adapter-protocols v1.5; codified gate check (ll) v1.43).
...
2. After each fix burst: run `bash scripts/check-spec-counts.sh` (must exit 0 — gate v1.43, checks a–z + aa, bb, cc, dd, ee, ff, gg, hh, ii, jj, kk, ll + o.ii, ~41 sub-assertions; check (gg) is D-SEC evaluator-completeness guard; check (hh) is economy-conservation BC-routing guard; check (ii) is visionOS/OpenXR dedicated-code routing guard; check (jj) is D-SEC no-DEGRADED-path consistency guard; check (kk) is W-code identifier-resolution guard; check (ll) is T2 comparison-method consistency guard). Machine-enforced spec-integrity gate. NEVER credit a pass without a green gate run.
...
**CI-gate contract:** `scripts/check-spec-counts.sh` v1.43 on `main` — checks a–z + aa, bb, cc, dd, ee, ff, gg, hh, ii, jj, kk, ll + o.ii (~41 sub-assertions). Key guards: (ee) corpus-wide genre-profile schema; (ff) L2-INDEX 8 registry counts; (w) GENERAL DI-007 context (playtest keyword required); (gg) D-SEC evaluator-completeness (4 sub-predicates); (hh) economy-conservation BC routing; (ii) visionOS/OpenXR dedicated-code routing; (jj) D-SEC no-DEGRADED-path; (kk) W-code identifier-resolution guard; (ll) T2 comparison-method consistency guard (adapter-protocols.md). MUST exit 0 before crediting any pass.
...
| phase-1d-adversarial | Adversarial spec convergence (>=3 clean passes); Passes 1–61 resolved (see phase-1-log.md + convergence table below); 3 streak breaks at P46/49/51; security-burst tails at P43/44/53; latest: Pass 61 FINDINGS (F61-01: F40-01 T2=snapshot-structured-diff not propagated to adapter-protocols.md §2.3 + §2.5 ReplayResult enum — fixed adapter-protocols v1.5; codified gate (ll) v1.43); **COUNTER: 0/3**; Pass 62 = restart #3 clean #1 candidate | IN PROGRESS |
...
| T9 | Phase-1d adversarial spec convergence — Passes 1–61 resolved; Pass 61 FINDINGS (F61-01: F40-01 T2=snapshot-structured-diff not propagated to adapter-protocols.md §2.3 tier table + §2.5 ReplayResult enum — fixed adapter-protocols v1.5; codified gate check (ll) v1.43); counter 0/3; Pass 62 = restart #3 clean #1 candidate | **IN PROGRESS** |
...
**Date:** 2026-06-13
**Phase:** 1 — Spec Crystallization IN PROGRESS. Steps 1–8 DONE + Phase-1d Passes 1–61 DONE.
**Phase-1d Pass 61:** FINDINGS (0C/1I/0obs). F61-01(I,process-gap→CODIFIED): F40-01 established T2 (same-machine) replay comparison = `snapshot-structured-diff` across BC-1.12.002/003, BC-3.03.004, methodology §D-REPLAY — but adapter-protocols.md was skipped. §2.3 still mapped `same-machine → snapshot-hash-diff`; §2.5 ReplayResult enum omitted `snapshot-structured-diff`. Fixed: adapter-protocols.md v1.4→v1.5 (§2.3 same-machine row corrected; §2.5 enum extended to all 3 methods). Codified: gate check (ll) v1.42→v1.43 (T2 comparison-method consistency; Assertion A = same-machine never maps to snapshot-hash-diff; Assertion B = ReplayResult enum contains all 3 methods, anchored on unique `"method":...tolerance-window` line). Note: Assertion B had initial mis-scoping bug caught by orchestrator's own gate run; re-anchored by architect.
**Clean-pass counter: 0/3 (counter stays — findings pass; restart #3 still seeking clean #1).** Pass 62 = restart #3 candidate clean #1.
**Next action:** `phase-1d-adversarial` — **Pass 62** (candidate clean #1, restart #3 continues). Spec FROZEN after F61 fix burst. FU-007/FU-008/FU-010/FU-011/FU-012/FU-013/FU-014/FU-016/FU-017/FU-018/FU-019/FU-020/FU-021/FU-022/FU-023/FU-024 open (non-blocking). FU-015 CLOSED (F51-02).
...
**Spec totals:** 193 BCs / 267 error codes (258 active) / 41 NFRs / 15 caps / 13 subsystems / 13 DI invariants / P0=127/P1=42/P2=24 / CI gate v1.43 (~41 sub-assertions). Error codes UNCHANGED (267); Warning Codes registry carries 4 W-codes (NOT counted in 267). §3.1 D-SEC = 2 allowed values (GREEN, BLOCKED); check (s) verifies 33 dim-value pairs.
**Version bumps Pass 61:** adapter-protocols.md v1.5 (F61-01: §2.3 same-machine→snapshot-structured-diff; §2.5 ReplayResult enum all 3 methods). CI gate v1.42→v1.43 (check (ll) added: T2 comparison-method consistency). BC count UNCHANGED (193). Spec totals UNCHANGED.
...
**main branch:** CI gate v1.43 committed (check (ll) T2 comparison-method consistency guard). Step history: see phase-1-log.md.
