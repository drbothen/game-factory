...
**Current clean-pass counter: 0/3** (restart #3 still seeking clean #1). Next action: Pass 55 (candidate clean #1). Economy-routing class gated (check hh v1.39).
...
2. After each fix burst: run `bash scripts/check-spec-counts.sh` (must exit 0 — gate v1.39, checks a–z + aa, bb, cc, dd, ee, ff, gg, hh + o.ii, ~38 sub-assertions; check (gg) is D-SEC evaluator-completeness guard; check (hh) is economy-conservation BC-routing guard). Machine-enforced spec-integrity gate. NEVER credit a pass without a green gate run.
...
**CI-gate contract:** `scripts/check-spec-counts.sh` v1.39 (checks a–z + aa, bb, cc, dd, ee, ff, gg, hh + o.ii, ~38 sub-assertions) on `main`. Purpose: prevent count-drift, vocabulary regression, and fix-induced cross-spec inconsistency across BC/NFR/error-taxonomy/priority/VP/studio/seam/dimension/creative-gate/no-variant-code/D-SEC-predicate/active-BC-stale-status/genre-profile-schema-gate-integrity/L2-INDEX-registry/D-SEC-evaluator-completeness/economy-conservation-BC-routing surfaces. Check (ee) is CORPUS-WIDE (scans all 194 BC files). Check (ff) validates L2-INDEX ID Registry Summary (8 ID-format counts) + Priority Distribution total vs source files. Check (w) is a GENERAL DI-007-context guard (requires playtest-domain keyword in ±10-line window of any operative DI-007 citation). Check (gg) asserts BC-7.11.001 references all 4 D-SEC sub-predicates (server-authority/anti-cheat/moderation/secrets). Check (hh) flags any operative line co-locating BC-6.02.001 with an economy-conservation keyword. MUST be green before any pass is credited.
...
| T9 | Phase-1d adversarial spec convergence — Passes 1–53 resolved; Pass 54 FINDINGS (2I, economy-routing + replay-degradation, fixed; economy-routing gated hh); counter 0/3; Pass 55 = restart #3 clean #1 candidate | **IN PROGRESS** |
...
**Date:** 2026-06-10
**Phase:** 1 — Spec Crystallization IN PROGRESS. Steps 1–8 DONE + Phase-1d Passes 1–54 DONE.
**Phase-1d Pass 54:** FINDINGS (0C/2I/2obs). F54-01(I): 4 SS-09 monetization BCs (BC-11.04.001/002, BC-11.02.002/003) cited BC-6.02.001 (Design Intent Reachability) for economy-conservation dependency → corrected to BC-6.01.001 (Economy Conservation Invariant); [process-gap] CI check (hh) added (gate v1.39). F54-02(I): T2→T3 replay degradation path (BC-3.03.004 EC-002) handed off to T3 tolerance comparison with no tolerance_spec source → fixed fail-closed: block E-REPLAY-002 unless T2 golden declares shadow tolerance_spec (BC-3.03.004/005/008 updated). OBS-54-A: VP-009 (+VP-008) back-ref title staleness propagated to BC-6.01.002 and verification-architecture.md — corrected.
**Clean-pass counter: 0/3 (counter unchanged — findings pass; restart #3 still seeking clean #1).** Pass 55 = restart #3 candidate clean #1.
**Next action:** `phase-1d-adversarial` — **Pass 55** (candidate clean #1, restart #3 continues). Spec FROZEN after F54 fixes. FU-007/FU-008/FU-010/FU-011/FU-012/FU-013/FU-014/FU-016/FU-017/FU-018 open (non-blocking). FU-015 CLOSED (F51-02).
...
**Spec totals:** 193 BCs / 267 error codes (258 active) / 41 NFRs / 15 caps / 13 subsystems / 13 DI invariants / priority P0=127/P1=42/P2=24 / CI gate v1.39 (checks a–z + aa, bb, cc, dd, ee, ff, gg, hh + o.ii, ~38 sub-assertions; check ee CORPUS-WIDE; check ff L2-INDEX registry integrity; check (w) GENERAL DI-007 context guard; check (gg) D-SEC evaluator-completeness guard; check (hh) economy-conservation BC-routing guard). Totals UNCHANGED (content-only fixes; no new BCs/codes/NFRs).
**Version bumps Pass 54:** BC-11.04.001 v1.1, BC-11.04.002 v1.2, BC-11.02.002 v1.1, BC-11.02.003 v1.2 (F54-01: economy BC-routing); BC-3.03.004 v1.1, BC-3.03.005 v1.1, BC-3.03.008 v1.2 (F54-02: T2→T3 replay degradation fail-closed); BC-6.01.002 v1.1, verification-architecture.md v1.1 (OBS-54-A: VP-009+VP-008 title staleness). CI gate v1.39 (check hh added). main at 306a536 (v1.39).
**Open FUs:** FU-007 (non-blocking), FU-008 (non-blocking), FU-010 (non-blocking), FU-011 (non-blocking), FU-012 (non-blocking; target Phase-6), FU-013 (OBS-47-A/49; non-blocking), FU-014 (OBS-47-B; non-blocking), FU-016 (OBS-1 Pass-50; cosmetic; non-blocking), FU-017 (OBS-51-A; non-blocking), FU-018 (OBS-51-B; non-blocking; verify). FU-009 CLOSED (Pass-24). FU-015 CLOSED (F51-02).
...
**main branch:** IN SYNC with origin/main at 306a536 (v1.39 gate; check (hh) economy-conservation BC-routing guard).
