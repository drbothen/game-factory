...
**Current clean-pass counter: 0/3** (Pass 52 FINDINGS — restart #3 has not yet produced clean #1). Next action: dispatch FRESH-CONTEXT adversary for Pass 53 (candidate clean #1). The DI-007 mis-anchor class (5th instance found in Canon-KB) is now gated GENERALLY (check w v1.37).
...
2. After each fix burst: run `bash scripts/check-spec-counts.sh` (must exit 0 — gate v1.37, checks a–z + aa, bb, cc, dd, ee, ff + o.ii, ~38 sub-assertions; check (w) is now a GENERAL DI-007 context guard). Machine-enforced spec-integrity gate. NEVER credit a pass without a green gate run.
...
**CI-gate contract:** `scripts/check-spec-counts.sh` v1.37 (checks a–z + aa, bb, cc, dd, ee, ff + o.ii, ~38 sub-assertions) on `main`. Purpose: prevent count-drift, vocabulary regression, and fix-induced cross-spec inconsistency across BC/NFR/error-taxonomy/priority/VP/studio/seam/dimension/creative-gate/no-variant-code/D-SEC-predicate/active-BC-stale-status/genre-profile-schema-gate-integrity/L2-INDEX-registry surfaces. Check (ee) is CORPUS-WIDE (scans all 194 BC files). Check (ff) validates L2-INDEX ID Registry Summary (8 ID-format counts) + Priority Distribution total vs source files. Check (w) is now a GENERAL DI-007-context guard (requires playtest-domain keyword in ±10-line window of any operative DI-007 citation). MUST be green before any pass is credited.
...
| T9 | Phase-1d adversarial spec convergence — Passes 1–51 resolved; Pass 52 FINDINGS (1I, DI-007 mis-anchor, gated); counter 0/3; Pass 53 = restart #3 clean #1 candidate | **IN PROGRESS** |
...
**Date:** 2026-06-10
**Phase:** 1 — Spec Crystallization IN PROGRESS. Steps 1–8 DONE + Phase-1d Passes 1–52 DONE.
**Phase-1d Pass 52:** FINDINGS (0C/1I/4obs). F52-01(I mis-anchor): BC-12.12.003 EC-006 spuriously cited DI-007 (playtest invariant) as justification for the Canon-KB contradiction-skip on ambiguous-by-design relationships — 5th instance of I-PASS32-01 class; escaped prior check (w) because that check only scoped cinematic keywords. Fixed: EC-006 wording updated to cite canon_tier: ambiguous-by-design per BC-12.12.001 §Invariants (v1.3); DI-007 operative reference removed. Check (w) generalized to require ANY operative DI-007 citation to have a playtest-domain keyword in its ±10-line window (gate v1.37). 4obs were positive confirmations (D-SEC/F51/F40 propagation verified sound) — no corrective action.
**Clean-pass counter: 0/3 (counter unchanged — findings pass; restart #3 has not yet produced clean #1).** Pass 53 = restart #3 candidate clean #1.
**Next action:** `phase-1d-adversarial` — **Pass 53** (candidate clean #1, restart #3 continues). Spec FROZEN after F52 fixes. FU-007/FU-008/FU-010/FU-011/FU-012/FU-013/FU-014/FU-016/FU-017/FU-018 open (non-blocking). FU-015 CLOSED (F51-02).
...
**Spec totals:** 193 BCs / 267 error codes (258 active) / 41 NFRs / 15 caps / 13 subsystems / 13 DI invariants / priority P0=127/P1=42/P2=24 / CI gate v1.37 (checks a–z + aa, bb, cc, dd, ee, ff + o.ii, ~38 sub-assertions; check ee CORPUS-WIDE; check ff L2-INDEX registry integrity; check (w) GENERAL DI-007 context guard). Totals UNCHANGED (content-only fix; no new BCs/codes/NFRs).
**Version bumps Pass 52:** BC-12.12.003 v1.3 (F52-01: EC-006 spurious DI-007 reference → canon_tier ambiguous-by-design). CI gate v1.37 (check w broadened). main at f833d0e (v1.37).
...
**main branch:** IN SYNC with origin/main at f833d0e (v1.37 gate; check (w) generalized to GENERAL DI-007 context guard).
